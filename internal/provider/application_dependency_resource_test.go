		...
		Steps: []resource.TestStep{
			// Create dependency with when_accessing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair("keycard_application_dependency.test", "when_accessing.0", "keycard_resource.additional1", "id"),
					resource.TestCheckResourceAttrPair("keycard_application_dependency.test", "when_accessing.1", "keycard_resource.additional2", "id"),
				),
			},
			// ImportState testing
			{
